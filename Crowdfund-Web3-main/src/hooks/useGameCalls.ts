...
export function useGameCalls() {
  ...
  const isContractDeployed = () => {
    return (GAME_PACKAGE_ID as string) !== "0x0" && 
  ...
  const createGame = async () => {
    if (!isContractDeployed()) {
      throw new Error("Game contract chưa được deploy! Vui lòng deploy smart contract và cập nhật GAME_PACKAGE_ID trong src/constants/index.ts");
    }
